      ...
      protected function init(param1:TweenLite, param2:Array, param3:Boolean) : void
      {
         ...
         while(_loc4_ < param2.length)
         {
            for(_loc5_ in param2[_loc4_])
            {
               ...
               {
                  _loc6_[_loc5_] = [param1.target[_loc5_]];
               }
               if(typeof param2[_loc4_][_loc5_] == "number")
               {
                  _loc6_[_loc5_].push(param2[_loc4_][_loc5_]);
               }
               else
               {
                  _loc6_[_loc5_].push(param1.target[_loc5_] + Number(param2[_loc4_][_loc5_]));
               }
            }
            _loc4_++;
         }
         for(_loc5_ in _loc6_)
         {
            ...
            if(param1.exposedVars[_loc5_] != undefined)
            {
               if(typeof param1.exposedVars[_loc5_] == "number")
               {
                  _loc6_[_loc5_].push(param1.exposedVars[_loc5_]);
               }
               else
               {
                  _loc6_[_loc5_].push(param1.target[_loc5_] + Number(param1.exposedVars[_loc5_]));
               }
               delete param1.exposedVars[_loc5_];
               _loc4_ = param1.tweens.length - 1;
               while(_loc4_ > -1)
               {
                  if(param1.tweens[_loc4_].name == _loc5_)
                  {
                     param1.tweens.splice(_loc4_,1);
                  }
                  _loc4_--;
               }
            }
         }
         this._beziers = parseBeziers(_loc6_,param3);
      }
      ...
      override public function set changeFactor(param1:Number) : void
      {
         ...
         if(this._orient)
         {
            ...
            while(_loc2_ < this._orientData.length)
            {
               ...
               _loc14_ = Number(Number(_loc13_[3]));
